...
void ILI9341_SetWindow(uint16_t start_x, uint16_t start_y, uint16_t end_x, uint16_t end_y)
{
    ...
    ILI9341_WriteCommand(0x2A); // CASET
        ...
        ILI9341_WriteCommand(0x2b); // CASET

        uint8_t data2[] = { 	(start_y >> 8),
							( 0xFF & start_y),
							(end_y>>8),
							(0xFF & end_y),
						 };
...
void ILI9341_DrawBitmap(uint16_t w, uint16_t h, uint16_t s)
{
	ILI9341_Select();
	ILI9341_WriteCommand(0x2c);
	HAL_GPIO_WritePin(ILI9341_DC_GPIO_Port, ILI9341_DC_Pin, GPIO_PIN_SET); 	//DC_H();
	//ConvHL(s, (int32_t)w*h*2);
	uint8_t data[] = { s >> 8, s & 0xFF };
	ILI9341_WriteData(data, sizeof(data));
